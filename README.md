...
## 栗子商城
**担任角色**：后端核心开发（全栈适配）
**技术栈**：Java 17、Spring Boot 3.x、MySQL、MyBatis/MyBatis-Plus、Redis、Shiro、Netty、Caffeine、RocketMQ、Maven、Elasticsearch
**工具链**：Lombok、MapStruct、Apache Commons、Jackson

**项目描述**：独立耗时3个月从零搭建前后端分离B2C电商单体项目，覆盖商品、购物车、订单、权限、在线客服等核心业务；完成数据库设计、接口开发、性能优化全流程开发
...
3. 数据层优化：使用 MyBatis-Plus 简化单表操作，自定义 ResultMap 实现多表查询；通过 SQL 索引优化，降低数据库 IO 开销，提升数据查询效率; 基于ES搭建商品全文检索体系，实现商品文档索引启动类、封装ES相关接口，实现商品高命中精准搜索，实现商品数据查询与数据库解耦，提升系统QPS。
